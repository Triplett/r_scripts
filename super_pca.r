...
# OTU Abundancy Matrix
file = commandArgs(trailingOnly=TRUE)[1]

# tab delim file with header and the number of samples for each test. Or "none."
# What test?
samples = commandArgs(trailingOnly=TRUE)[2]

# if there are cases and controls, how many cases are there? If not, 0.
num_cases = commandArgs(trailingOnly=TRUE)[3]

# are there pvalues and qvalues in the input table? yes or no.
ttest = commandArgs(trailingOnly=TRUE)[4]

# do you want to log transform the data before doing pca? yes or no.
trans = commandArgs(trailingOnly=TRUE)[5]

# do you want to plot 3D graphs?  yes or no.
threeD = commandArgs(trailingOnly=TRUE)[6]

# do you want to print a table with the rotation coordinates? yes or no
print_table = commandArgs(trailingOnly=TRUE)[7]
